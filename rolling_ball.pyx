...
def roll_ball(ball, np.ndarray array,invert=True):
    """
    ...
    cdef unsigned int height = array.shape[0]
    cdef unsigned int width  = array.shape[1]
    cdef np.ndarray[np.float32_t, ndim=1] pixels = numpy.float32(array.flatten())
    cdef unsigned int ball_width = ball.width
    cdef unsigned int radius = ball_width / 2
    cdef np.ndarray[double, ndim=1] cache = numpy.zeros(width * ball_width)
    
    cdef unsigned int next_line_to_write_in_cache,next_line_to_read,src,dest
    cdef unsigned int y,x,p,yp,cache_pointer,bp,xp
    cdef int y_0,y_ball_0,y_end,x_0,x_ball_0,x_end,z_reduced
    cdef double z_min
    ...
    return numpy.reshape(pixels, (height,width))
